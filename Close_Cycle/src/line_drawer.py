...
class LineDrawer(Widget):
    ...
    def animate_lines_grow_and_shift_positive(self, line_id_a,line_id_b, hs, duration=1):
        linea = self.ids[line_id_a]
        lineb = self.ids[line_id_b]
        animA=Animation(points=[linea.points[0]+hs, linea.points[1]-hs, linea.points[2]+hs, lineb.points[3]+hs], duration=duration)
    ...
    def animate_lines_grow_and_shift_positive_negative(self, line_id_a,line_id_b, hs, duration=1):
        ...
        lineb = self.ids[line_id_b]
        animA=Animation(points=[linea.points[0]+hs, linea.points[1]-hs, linea.points[2]+hs, lineb.points[3]+hs], duration=duration)
        new_points_a = [int(linea.points[0])+hs, int(linea.points[1]-hs), int(linea.points[2]+hs), int(lineb.points[3]-hs)]
        animA.start(linea)
        animB=Animation(points=[lineb.points[0], lineb.points[1]-hs, linea.points[2] + hs, lineb.points[3]-hs], duration=duration)
        new_points_b = [int(lineb.points[0]+hs), int(lineb.points[1]-hs), int(linea.points[2]), int(lineb.points[3]-hs)]
        animB.start(lineb)
    ...
    def draw_line_connecting_two_lines_B(self, line_id1, line_id2, line_id3, duration=1):
        # Obtener las líneas según sus identificadores
        line1 = self.ids[line_id1]
        line2 = self.ids[line_id2]
        line3 = self.ids[line_id3]

        # Obtener los extremos de las dos líneas
        x1, y1 = line1.points[2], line1.points[3]
        x2, y2 = line2.points[0], line2.points[1]

        # Crear la animación para conectar las dos líneas desde los extremos
    ...
    def draw_base(self):
        ...
        self.draw_line(1070, 430, 1070, 470, 'h1a', color=(0, 0, 0, 1), width=2)
        #Line 1h2 union
        self.draw_line(1035, 400, 1070, 430, '1h2', color=(0.5, 0.5, 0.5, 1), width=1)
        #line 5h6 union 
        ...
        #Line 1h8
        self.draw_line(1070, 470, 1065, 500, '1h8', color=(0.5, 0.5, 0.5, 1), width=1)
        #line 2h9 union
